...
impl<B: Backend> Actor<B> for ConclaveActor<B> {
    ...
    fn forward(&self, _cfg: &Cfg, input: Input<B>) -> Output<B> {
        ...
        let ir_emb = self.ir_proj.forward(
            input.ir_features.narrow(1, 0, 1).reshape([n, nf])
        );                                                                   // [N, d_model]
        ...
        let pass_ids  = Tensor::<B, 1, Int>::arange(0..np as i64, &device)
            .unsqueeze_dim(0);                                               // [1, np]
        ...
        let policy      = policy_flat
            ...
            .unsqueeze_dim(2);                                               // [N, K, 1, num_passes]
